...
def test_model_artifact_structure():
    ...
    # 3. Validation
    assert isinstance(model, RandomForestClassifier), "Artifact is not a Random Forest Classifier"
    
    # Check if it's fitted (has estimators)
    # hasattr check is a robust way to see if a sklearn model is trained
    assert hasattr(model, "estimators_"), "Model appears to be untrained (no estimators)"
